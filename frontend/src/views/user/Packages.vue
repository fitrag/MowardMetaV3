<template>
  <div class="space-y-6">
    ...
    <div class="bg-gray-950 rounded-xl p-6 text-white">
      <h2 class="text-lg font-medium">Choose Your Package</h2>
      <p class="text-sm text-gray-400 mt-1">Select a package that fits your needs. Each purchase gives you credits to generate metadata.</p>
    </div>
    ...
    <div v-if="loading" class="flex items-center justify-center py-20">
      ...
      <span class="ml-2 text-sm text-gray-400">Loading packages...</span>
    </div>

    <!-- Empty State -->
    <div v-else-if="packages.length === 0" class="flex flex-col items-center justify-center py-20">
      <ShoppingBagIcon class="w-12 h-12 text-gray-200 mb-3" />
      <h3 class="text-sm font-medium text-gray-900 mb-1">No packages available</h3>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        ...
        <div class="p-6">
          <!-- Package Header -->
          <div class="mb-5">
            <div class="w-10 h-10 rounded-lg flex items-center justify-center mb-3" :class="getPackageIconBg(index)">
              <component :is="getPackageIcon(index)" class="w-5 h-5" :class="getPackageIconColor(index)" />
            </div>
          ...
          <div class="space-y-2.5 mb-6">
            ...
            <div v-if="pkg.durationDays" class="flex items-center gap-2.5">
              <ClockIcon class="w-4 h-4 text-gray-400 flex-shrink-0" />
              <span class="text-sm text-gray-700">{{ pkg.durationDays }} days validity</span>
            </div>
            <div class="flex items-center gap-2.5">
              <CheckCircleIcon class="w-4 h-4 text-emerald-600 flex-shrink-0" />
              <span class="text-sm text-gray-700 capitalize">{{ pkg.packageType }} package</span>
            </div>
          </div>
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="flex items-start gap-3">
          ...
          <div>
            <p class="text-sm font-medium text-gray-900">Choose a Package</p>
            <p class="text-xs text-gray-400 mt-0.5">Select the package that fits your needs</p>
          </div>
        </div>
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-gray-400 mt-0.5">Choose payment method and follow instructions</p>
          </div>
        </div>
        <div class="flex items-start gap-3">
          ...
          <div>
            <p class="text-sm font-medium text-gray-900">Start Generating</p>
            <p class="text-xs text-gray-400 mt-0.5">Credits are added once payment is approved</p>
          </div>
        </div>
      </div>
    </div>
...
import { ShoppingBagIcon, CheckCircleIcon, ClockIcon, StarIcon, CreditCardIcon, SparklesIcon, BoltIcon, XMarkIcon } from '@heroicons/vue/24/outline'
...
onMounted(async () => {
  try {
    packages.value = await userService.getPackages()
  } catch (error) {
...
const isFeatured = (pkg) => {
  return pkg.creditAmount >= 50 || pkg.packageType === 'subscription'
}

const getPackageIcon = (index) => {
  const icons = [CreditCardIcon, SparklesIcon, BoltIcon]
  return icons[index % icons.length]
}

const getPackageIconBg = (index) => {
  const bgs = ['bg-violet-100', 'bg-amber-100', 'bg-emerald-100']
  return bgs[index % bgs.length]
}

const getPackageIconColor = (index) => {
  const colors = ['text-violet-700', 'text-amber-700', 'text-emerald-700']
